...
export function SSOHandler({ children }) {
  ...
  useEffect(() => {
    ...
    const performSsoLogin = async () => {
      const baseApiUrl = (import.meta.env.VITE_API_BASE_URL || 'https://api.beam.safesiteworks.com/development/m3infrastructure').replace(/\/$/, '');
      const endpoints = [
      ...
      if (baseApiUrl.includes('localhost')) {
        endpoints.push('http://localhost:3000/auth/sso-login');
        endpoints.push('http://localhost:3000/api/auth/sso-login');
      }
      ...
      if (success && data) {
        ...
        // Standard keys (scopedStorage will dynamically attach m3infra_ prefix)
